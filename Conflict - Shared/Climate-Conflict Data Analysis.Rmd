...
```{r descriptive-analysis, eval = FALSE}
...
battles_twowayFE_clreg_1to0 <- feols(
  battles_explosions ~ l(temp2m_c_std, 0:1) + l(prcp_mm_std, 0:1) | 
    region_id + year,
  data = region_year %>% filter(year < 2021),
  panel.id = ~region_id + year,
...
etable(battles_twowayFE_1to0, battles_twowayFE_clreg_1to0)
feols(
  battles_explosions ~ l(temp2m_c, 0:1) + l(prcp_mm, 0:1) | 
    region_id + year,
  data = region_year %>% filter(year < 2021),
  panel.id = ~region_id + year,
  cluster = ~region_id
...
coefplot(protests_twowayFE_1to0)
coefplot(battles_twowayFE_1to0)
coefplot(violciv_twowayFE_1to0)
```
